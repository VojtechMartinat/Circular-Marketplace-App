...
const getArticleByOrderId = asyncErrorWrapper(async (req, res, next) => {
    const { id: orderID } = req.params;
    console.log(`Fetching article for orderID: ${orderID}`);
    ...
    const article = await Article.findOne({ where: { orderID } });
    if (!article) {
        return next(new APIError(`No article found for orderID: ${orderID}`, 404));
    }
